...
model_auto_hetero = load_model("models/FMNIST_auto_hetero.h5")
...
batch_size_for_reinjection = 512
number_of_reinjections = 10
...
for step in range(25000): 
    ...
    if (step % 100 == 0):
        model.save_weights("weights/fmnist_target_step:" + str(step) + "_acctrain:" + str(acc_train) + "_acctest:" + str(acc_test) + ".hdf5")
    
    
#model.load_weights("weights/fmnist_target_step:8700_acctrain:0.8422857142720903_acctest:0.8328.hdf5")    
#model.save("models/FMNIST_target_reinj_validate.h5")    
#    
#acc_train = model.evaluate(X_train, Y_train, verbose=0)[1]
#acc_test = model.evaluate(X_test, Y_test, verbose=0)[1]
#print("Model, accuracy on train set: " + str(acc_train))
